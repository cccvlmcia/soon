import {addBatchUserAuth, getUserAuthInfo, getUserAuthList, removeUserAuth} from "@user/service/UserAuthService";
...
export default async function (fastify: FastifyInstance) {
  ...
  fastify.post("/", async (req: FastifyRequest<{Params: {userid: number}; Body: {authids: string[]}}>, reply: FastifyReply) => {
    const {userid} = req.params;
    const {authids} = req.body;
    const auth = await addBatchUserAuth(userid, authids);
    reply.send(auth);
  });
